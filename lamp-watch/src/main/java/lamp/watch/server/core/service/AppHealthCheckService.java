...
import lamp.watch.server.core.domain.AppHealthStatusCode;
...
public class AppHealthCheckService {
	...
	public void checkHealth(WatchedApp watchedApp) {
		...
		if (StringUtils.isNotBlank(url)) {
			...
			appStatusService.updateStatus(watchedApp, AppHealthStatus.of(code, description), LocalDateTime.now());
		}
	}
	...
	protected Map<String, Object> getHealth(String url) {
		...
		try {
			...
			health = new HashMap<>();
			health.put(CODE, AppHealthStatusCode.OUT_OF_SERVICE.name());
