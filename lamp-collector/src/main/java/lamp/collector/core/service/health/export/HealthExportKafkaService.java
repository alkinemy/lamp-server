...
@Slf4j
public class HealthExportKafkaService implements HealthExportService {

	private Producer<String, CollectionTarget> producer;
	private String topic;

	public HealthExportKafkaService(KafkaProducerProperties kafkaProperties) {
	...
	public void export(CollectionTarget collectionTarget, TargetHealth health) {
		String key = new StringBuilder().append(collectionTarget.getId()).append('-').append(health.getTimestamp()).toString();

		ProducerRecord<String, CollectionTarget> data = new ProducerRecord(topic, key, health);
